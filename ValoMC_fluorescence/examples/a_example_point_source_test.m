...
% Create a finer mesh
h_disc=0.4;
x_arr = h_disc/2:h_disc:10-h_disc/2;
y_arr = h_disc/2:h_disc:10-h_disc/2;
z_arr = h_disc/2:h_disc:10-h_disc/2;
% h_disc=0.2;
...
vmcmedium.scattering_coefficient_ex = 10.0;
vmcmedium.scattering_coefficient_em = 5.0;
vmcmedium.absorption_coefficient_ex_sol = 0.1; % repeat six times
vmcmedium.absorption_coefficient_ex_f = 0.4; % repeat six times
vmcmedium.absorption_coefficient_em_sol = 0.5; % repeat six times
...
Point_LS=[5.0+h_disc/4 5.0+h_disc/4 10];
%Point_LS=[0 0 0];
x_coord=vmcmesh.r(:,1);
y_coord=vmcmesh.r(:,2);
z_coord=vmcmesh.r(:,3);
...
vmcboundary.exterior_refractive_index = 1.0;
figure(1);clf
...
fn1=reshape(solution.grid_fluence1(:,13,:),size(solution.grid_fluence1,1),size(solution.grid_fluence1,3));
figure(4);clf
imagesc(z_arr',x_arr', log(fn1));
%J_im1 = imrotate(J_im,90,'bilinear','crop');
xlabel('z [mm]');
ylabel('x [mm]');
%figure
