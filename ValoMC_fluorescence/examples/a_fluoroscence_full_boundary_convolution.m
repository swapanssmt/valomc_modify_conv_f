...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
options.frequency=0;    
options.photon_count_ex = 1e8;
options.photon_count_em = 1000;
options.Qyield_f = 1;
[solution_ex,solution_em] = ValoMC(vmcmesh, vmcmedium, vmcboundary,options);
%node_solution=nodalbasis3D(vmcmesh,solution);
%nod_solution=nodalbasis(vmcmesh,solution);
convolution_fluence_ex=solution_ex.element_fluence;
convolution_fluence_em=solution_ex.element_fluence.*solution_em.F_element_fluence;
...
first = reshape(solution_ex.element_fluence(1:nvoxels), nx,ny,nz);
second = reshape(solution_ex.element_fluence(nvoxels+1:2*nvoxels),nx,ny,nz);
third = reshape(solution_ex.element_fluence(nvoxels*2+1:3*nvoxels),nx,ny,nz);
fourth = reshape(solution_ex.element_fluence(nvoxels*3+1:4*nvoxels),nx,ny,nz);
fifth = reshape(solution_ex.element_fluence(nvoxels*4+1:5*nvoxels),nx,ny,nz);
sixth = reshape(solution_ex.element_fluence(nvoxels*5+1:6*nvoxels),nx,ny,nz);
solution_ex.grid_fluence = (first+second+third+fourth+fifth+sixth)/6;
%solution_ex.grid_fluence = solution_ex.grid_fluence/max(max(max(solution_ex.grid_fluence)));
fn=reshape(solution_ex.grid_fluence(:,17,:),size(solution_ex.grid_fluence,1),size(solution_ex.grid_fluence,3));
figure(3);clf
...
% snapnow;
%% for flouroscence
first1 = reshape(solution_em.F_element_fluence(1:nvoxels), nx,ny,nz);
second2 = reshape(solution_em.F_element_fluence(nvoxels+1:2*nvoxels),nx,ny,nz);
third3 = reshape(solution_em.F_element_fluence(nvoxels*2+1:3*nvoxels),nx,ny,nz);
fourth4 = reshape(solution_em.F_element_fluence(nvoxels*3+1:4*nvoxels),nx,ny,nz);
fifth5 = reshape(solution_em.F_element_fluence(nvoxels*4+1:5*nvoxels),nx,ny,nz);
sixth6 = reshape(solution_em.F_element_fluence(nvoxels*5+1:6*nvoxels),nx,ny,nz);
solution_em.grid_fluence1 = (first1+second2+third3+fourth4+fifth5+sixth6)/6;
%solution_em.grid_fluence1 = solution_em.grid_fluence1/max(max(max(solution_em.grid_fluence1)));
%fn2=reshape(sum(solution.grid_fluence1,2),size(solution.grid_fluence1,1),size(solution.grid_fluence1,3));
fn1=reshape(solution_em.grid_fluence1(:,17,:),size(solution_em.grid_fluence1,1),size(solution_em.grid_fluence1,3));
figure(4);clf
